...
import { type Config, configSchemaDefaults } from '../config/config.schema.ts'
      ...
      return renderTemplate({
        ...
        object: {
          ...
          $NUMBERS: change.pullRequests
            .map(({ number }) => `#${number}`)
            .join(
              params.config['change-numbers-separator'] ??
                configSchemaDefaults['change-numbers-separator'],
            ),
