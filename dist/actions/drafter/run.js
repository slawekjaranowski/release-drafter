...
import { _ as evaluateCategories, a as COERCE, b as getVersionResolverCategories, c as PRERELEASE_LOOSE, d as formatFullVersion, f as parse, g as commonConfigSchema, h as configSchemaDefaults, i as satisfies, l as compareIdentifiers, m as tryParse$1, n as mergeInputAndConfig, o as COERCE_FULL, p as safeRegex, r as normalizeRange, s as PRERELEASE, t as getReleaseDrafterConfig, u as formatComparableVersion, v as filterPullRequestsByPreCategories, x as needsPullRequestChangedFiles, y as getChangelogCategories } from "../../chunks/get-release-drafter-config.js";
...
var groupChanges = (params) => {
	...
	for (const [index, pullRequest] of pullRequests.entries()) {
		...
		const key = `rule ${match.index} ${match.group}`;
...
var matchRule = (pullRequest, rules) => {
	for (const [index, rule] of rules.entries()) {
		const group = rule.pattern.exec(pullRequest.title)?.groups?.group;
...
var mergeTitle = (params) => {
	...
	const object = { $GROUP: newest.group ?? "" };
...
var pullRequestToString = (params) => params.changes.map((change) => {
	...
	return renderTemplate({
		...
		object: {
			...
			$NUMBERS: change.pullRequests.map(({ number }) => `#${number}`).join(params.config["change-numbers-separator"] ?? configSchemaDefaults["change-numbers-separator"]),
